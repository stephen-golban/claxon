...
export function MessageComposer({ vehicle, onSend, onCancel }: MessageComposerProps) {
  ...
  const TemplateButton = ({ template }: { template: (typeof mockTemplates)[0] }) => (
    <Button
        ...
        <Card>
          <CardContent className="p-4">
            <View className="flex-row items-start gap-3">
              <Avatar alt={`${vehicle.vehicle.brand} ${vehicle.vehicle.model}`} className="h-12 w-12">
                <AvatarFallback style={{ backgroundColor: color?.rgba || "#ccc" }}>
                  <Text className="text-sm font-bold text-white">{vehicle.vehicle.brand.charAt(0)}</Text>
                </AvatarFallback>
              </Avatar>
              ...
              <View className="flex-1 gap-1">
                ...
                <Text className="text-sm text-muted-foreground">
                  {vehicle.vehicle.manufacture_year} • {color?.description || vehicle.vehicle.color}
                </Text>
                <Text className="text-sm font-medium">🇲🇩 {vehicle.vehicle.plate_number}</Text>
                <Text className="text-sm text-muted-foreground">To: {ownerName}</Text>
              </View>
            </View>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent className="gap-3">
            ...
            <View>
              ...
              {messageType === "custom" && (
                <TextField name="message" placeholder="Type your message here..." multiline numberOfLines={4} />
              )}
            </View>
          </CardContent>
        </Card>
