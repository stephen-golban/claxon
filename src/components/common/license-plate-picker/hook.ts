...
export const usePlateSections = (type: LicensePlateType): PlateSection[] => {
	return useMemo((): PlateSection[] => {
		const standardPlates = Object.entries(
			LICENSE_PLATE_TYPES.cars.standard,
		).map(([key, plate]) => ({
			key,
		...
		const governmentalPlates = governmentalPlateKeys.map((key) => {
			const plate =
				LICENSE_PLATE_TYPES.cars.special[
					key as keyof typeof LICENSE_PLATE_TYPES.cars.special
				];
			return {
				key,
