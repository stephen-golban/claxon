...
const LicensePlatePicker: React.FC<ILicensePlatePicker> = ({
	children,
	onTypeChange,
	type,
}) => {
	...
	const getCurrentPlateImage = useCallback(() => {
		...
		if (category === "cars") {
			if (subcategory === "standard") {
				return LICENSE_PLATE_TYPES.cars.standard[
					plateType as keyof typeof LICENSE_PLATE_TYPES.cars.standard
				]?.image;
			}
			if (subcategory === "regional") {
				return LICENSE_PLATE_TYPES.cars.regional[
					plateType as keyof typeof LICENSE_PLATE_TYPES.cars.regional
				]?.image;
			}
			if (subcategory === "special") {
				return LICENSE_PLATE_TYPES.cars.special[
					plateType as keyof typeof LICENSE_PLATE_TYPES.cars.special
				]?.image;
			}
		}
			...
			>
				<View className="flex-1 flex-row items-center">
					<Image
						source={item.image}
						contentFit="contain"
						style={{ width: 80, height: 48, marginRight: 16 }}
					/>
					<View className="flex-1">
						<Text
							className={cn(
								"text-base font-semibold",
								item.isSelected ? "text-primary" : "text-foreground",
							)}
						>
	...
	return (
		<>
			<View className="flex-row items-center gap-x-2">
				...
				<Button
					size="icon"
					className={cn("!h-16 !w-16 rounded-xl")}
					variant="outline"
					onPress={handleOpenBottomSheet}
				>
					<Image
						source={getCurrentPlateImage()}
						contentFit="contain"
