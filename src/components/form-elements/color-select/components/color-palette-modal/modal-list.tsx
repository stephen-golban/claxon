...
const DATA_WITH_PLACEHOLDER = [...VEHICLE_COLORS, { code: "placeholder", description: "Placeholder", rgba: null }];

const ModalList: React.FC<IModalList> = ({ onColorSelect, selectedColorIndex }) => {
	const renderItem: ListRenderItem<VehicleColor> = React.useCallback(
		({ item, index }) => {
			return (
				<ModalListItem item={item} onPress={() => onColorSelect(index)} isSelected={selectedColorIndex === index} />
			);
		},
