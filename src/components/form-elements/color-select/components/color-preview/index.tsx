...
const ColorPreview: React.FC<IColorPreview> = ({
	...
	const {
		...
		const isMulticolor =
			selectedColor?.code === "MUL" || selectedColor?.code === "CAM";
	...
	const correctedColor = LIGHT_COLORS.includes(selectedColor?.code || "")
		? "black"
		: "white";
			...
			<View className="relative w-full flex-1 flex-row items-center">
				...
				<View
					...
					className={cn(
						...
						error &&
							"border-destructive bg-destructive/10 text-destructive dark:bg-destructive/10",
					)}
				...
				>
					{isMulticolor && (
						<LinearGradient
							...
							colors={
								(selectedColor?.code === "MUL"
									? MULTICOLOR_COLORS
									: CAMOUFLAGE_COLORS) as [ColorValue, ColorValue]
							}
						/>
					)}
