...
export default function useBasePlate(props: BaseRenderPlateProps) {
	...
	const handleLeftChange = (text: string) => {
		...
		if (
			...
			(maskLeft?.value || "").replace(/\s/g, "").length
		) {
