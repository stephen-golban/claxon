...
export const PlateInputBox = React.forwardRef<TextInput, IPlateInputBox>(
	(props, ref) => {
		const {
			mask,
			type,
		...
		const placeholderTextColor =
			type === "cars.standard.public_transport"
				? "rgba(0,0,0,0.2)"
				: "lightgray";

		// For special plates (except diplomatic) on the left side, show only non-editable text
		const isSpecialPlateLeft =
			type.includes("cars.special") &&
			type !== "cars.special.diplomatic" &&
			side === "left";
		...
		return (
			<View className="flex-1 flex-row items-center">
				{isSpecialPlateLeft ? (
					<Text
						style={[styles.input, { lineHeight: 45, color }]}
