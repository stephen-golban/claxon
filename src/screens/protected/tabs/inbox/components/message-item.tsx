...
export function MessageItem({ message, onPress, onMarkAsRead }: MessageItemProps) {
	...
	return (
		<Pressable
			onPress={handlePress}
			className={cn("flex-row items-start gap-3 p-4", !message.read && "bg-muted/20", "active:bg-muted/40")}
		>
			{/* Avatar */}
			...
			<View className="flex-1 gap-1">
				...
				<View className="flex-row items-center justify-between">
					<View className="flex-row items-center gap-2 flex-1">
						<Text className={cn("font-medium text-sm", !message.read && "font-semibold")}>{displayName}</Text>
						{!message.read && <View className="h-2 w-2 bg-primary rounded-full" />}
					</View>
				...
				{/* Message Preview */}
				<Text className={cn("text-sm text-foreground mt-1", !message.read && "font-medium")} numberOfLines={2}>
