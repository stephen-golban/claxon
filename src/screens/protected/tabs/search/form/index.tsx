...
const SearchVehiclePlateForm: React.FC<ISearchVehiclePlateForm> = ({
	onSubmit,
	initialData,
}) => {
	const { t } = useTranslation();
	const { type, setType, resetForm, ...formMethods } =
		useVehiclePlateForm(initialData);
					...
					<Controller
						...
						render={({ field }) => (
							<LicensePlateField
								...
								onLeftChange={(txt) =>
									field.onChange({ ...field.value, left: txt })
								}
								onRightChange={(txt) =>
									field.onChange({ ...field.value, right: txt })
								}
							/>
						)}
					/>
