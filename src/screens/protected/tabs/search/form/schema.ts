...
export const searchVehiclePlateSchema = (
	leftLength: number,
	rightLength: number,
) =>
	z.object({
		plate: z.object({
			left: z
				.string()
				.min(leftLength, stringifyObjectValidate({ keyT: "errors:required" })),
			right: z
				.string()
...
export type SearchVehiclePlateFormData = z.infer<
	ReturnType<typeof searchVehiclePlateSchema>
> & {
