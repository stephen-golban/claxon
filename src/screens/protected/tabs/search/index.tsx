...
export function SearchTab() {
	...
	const [selectedVehicle, setSelectedVehicle] = useState<SearchResult | null>(
		null,
	);
	...
	const handleSearch = async (data: SearchVehiclePlateFormData) => {
		...
		setTimeout(() => {
			...
			const results = mockSearchResults.filter(
				(result) => result.vehicle.plate_number === plateNumber,
			);
	...
	if (showMessageComposer && selectedVehicle) {
		return (
			<Container>
				<Container.TopText
					title="Send Message"
					subtitle={`Send a claxon to ${selectedVehicle.vehicle.plate_number}`}
				/>
				<MessageComposer
